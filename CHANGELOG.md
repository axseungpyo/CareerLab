...
## v0.9.0 — 기업분석 고도화 Sprint 1~2 (2026-03-20)

### 기업분석 — 새 기능
- **추가** Serper 프로바이더 (Google SERP, Tavily 대비 53배 저렴 $0.30/1K)
- **추가** 웹 검색 ON/OFF 토글 — 분석별 웹 검색 선택, 기본 OFF (API 비용 절감)
- **추가** 채용공고 URL 자동 파싱 — URL 입력 → "자동 추출" → BeautifulSoup 크롤링
- **추가** 재분석 기능 — 상세 페이지에서 기존 분석 업데이트
- **추가** `PUT /api/company/{id}` — 키워드/요구사항 부분 업데이트 API

### 기업분석 — UX 개편
- **변경** 상세 페이지 4-탭 구조: 요약 / 요구사항 / 전략 / 리서치
- **추가** 요구사항 카테고리 필터 (전체/필수/우대/기술/소프트/경험/숨겨진)
- **추가** 키워드 인라인 편집: Badge X삭제 + 키워드 추가 input
- **추가** 요구사항 편집: hover 삭제 + 태그 선택 추가 폼
- **추가** 리서치 탭: 웹 검색 미사용 시 안내 메시지 + 카테고리별 표시

### 설정 (Settings)
- **추가** Serper 프로바이더 선택 + API Key 입력 + 설명 텍스트
- **수정** Serper API Key 마스킹/보존/상태 표시 (보안)

### 인프라
- **추가** `beautifulsoup4` 의존성 (URL 파싱용)
- **추가** Plan/Design 문서: company-analysis-v2

---

## v0.8.1 — UX 안정화 + 경쟁 분석 + 내보내기 확장 (2026-03-19)

### 경쟁사 분석
- **추가** 한국 3대 채용사이트 심층 분석: 사람인/자소설닷컴/잡코리아 (로그인 후 탐색)
- **추가** `docs/03-analysis/competitive-analysis-final.md` — 통합 분석 보고서 (Worxphere 생태계 발견)
- **추가** CareerLab Edge Function 아이디어 15건 도출 (Tier 1~3 우선순위)

### 프로필 개선
- **추가** 고등학교 학력: 요약 뷰 (대학교와 동일한 뷰/편집 토글) + 15개 계열 선택
- **개선** 대학교 편집: 3개 그룹(기본정보/전공·학점/재학기간) + 추가정보 접힘 섹션
- **개선** 경력 생성 폼: 모든 필드에 Label 추가 + gap-2→gap-4 간격 개선
- **변경** 경력 유형 기본값: "career" → "선택" placeholder
- **변경** 이수교과목 구분 기본값: "major_elective" → "선택" placeholder

### 자소서 관리
- **추가** 문항 순서 이동: ▲▼ 버튼으로 문항 위/아래 이동
- **추가** 글자수 프로그레스: 편집/읽기 모드 모두 하단 글자수 표시 + 초과 시 빨간색 경고
- **추가** 글자수 자동 파싱: 문항 텍스트에서 "700자", "최대 900자", "1500자 이내" 자동 감지
- **수정** 영문 글자수 제외: "(영문작성 시 1400자)" 패턴 무시, 한글 글자수만 반영
- **수정** DB 글자수 불일치 5건 수정 (문항 텍스트 기준으로 정정)
- **수정** 버전 삭제 "실패" 오류: api.delete가 204 No Content 응답을 JSON 파싱 시도 → undefined 반환으로 수정
- **변경** 자소서 상태 Select: "draft/final/submitted" → "작성중/완성/제출" 한글 표시

### 내보내기 확장
- **추가** 6가지 형식 지원: Word (.docx), PDF (.pdf), HTML (.html), Markdown (.md), 텍스트 (.txt), JSON (.json)
- **추가** 내보내기 DropdownMenu UI (Download 아이콘 + 형식 선택)
- **추가** 백엔드 `to_html()`, `to_json()`, `to_md()`, `to_txt()` exporter 메서드

### 기업분석 버그 수정
- **수정** `<a>` 중첩 hydration 에러: Card 내부 Link 중첩 → router.push로 변경
- **수정** 404 리소스 로드 실패: 중첩 Link prefetch 제거로 해소

### Select 컴포넌트 한글화
- **수정** base-ui Select에서 UUID/영어 코드가 trigger에 표시되는 문제 일괄 수정
- **수정** 대상: 면접(자소서 선택), 첨삭(자소서/문항 선택), 면접 비교(세션 A/B), 면접 모드, 자소서 상태

### 인프라
- **정리** 불필요 파일 삭제: PNG 스크린샷 145개(~88MB), .playwright-mcp 캐시, 중복 분석 파일 3개, 오래된 PDCA 스냅샷 2개
- **추가** `.env.jobsites` (경쟁사 로그인 정보, .gitignore 등록)

---

## v0.8.0 — UX 고도화 + 문서 정리 + 모델 통일 (2026-03-19)

### 프로필 개선
- **변경** 프로필 7탭: 기본정보/학력/이수교과목/경력/외국어·자격/자기소개/가져오기
- **변경** "Essay" 탭명 → "자기소개"로 변경
- **추가** 자기소개 탭: 성장배경(500자), 가치관(300자), 강점/약점(300자) + 글자수 카운터
- **추가** 취미/특기 분리, 존경인물 글자수 카운터 + 면접 빈출 힌트
- **변경** 주소 필드 → 한자이름(name_hanja)으로 변경
- **개선** 병역사항: status 영어→한글(복무완료/미필/비대상/면제), 레이아웃 개선
- **개선** 영어회화 필수자격: 전용 등록 Dialog (OPIc 9등급/TOEIC-Speaking 8레벨)
- **추가** 커스텀 DateSelect: 달력 팝업 내 연도 그리드 토글로 빠른 연도 선택

### 자소서 생성 UX 전면 리라이트
- **변경** 3스텝→2스텝 위저드: ①기업선택 → ②자소서 작성
- **추가** 기출문항 자동 로드: essay_questions DB + 삼성화재 2026 상반기 4문항 시드
- **추가** 문항별 개별 AI 생성 / 직접 작성 선택
- **추가** 문항별 1안/2안/3안 AI 생성 + 탭 전환 비교
- **추가** 생성 후 바로 편집 가능 (Textarea + 글자수 카운터)

### 자소서 관리
- **추가** 자소서 목록: 삭제 버튼 (confirm 후 일괄 삭제)
- **추가** 자소서 상세: 문항 추가/삭제/질문수정 기능
- **추가** 자소서 상세: 문항별 1안/2안/3안 버전 탭 + 새 버전 추가 (최대 3개)

### 기업분석 UX 개선
- **수정** 중복 버튼 제거: "첫 분석 시작" → "새 분석" 하나로 통합
- **추가** 분석 카드에 자소서 바로가기 아이콘

### LLM 모델 통일
- **변경** 전체 태스크를 Claude Sonnet 4.6으로 통일 (OpenAI 의존 제거)
- **추가** Claude JSON 응답 파싱: markdown code fence 자동 제거
- **추가** 임베딩 실패 시 graceful fallback (OpenAI 키 없어도 자소서 생성 가능)

### 문서 정리
- **이동** planning/ 9개 → docs/archive/planning/ (Phase 0 초기 기획, historical)
- **이동** 프롬프트 docs → docs/prompts/
- **추가** docs/README.md 인덱스
- **변경** CLAUDE.md: Search API(Brave→Tavily), Agent 전략(팀에이전트 우선), Phase 테이블
- **갱신** feature-backlog.md: v0.4~v0.7 완료 항목 처리 + v0.8 Roadmap

### 백엔드
- **추가** essay_questions 테이블 + CRUD API (4개 엔드포인트)
- **추가** DELETE /api/resume/{id} (자소서 삭제)
- **추가** DELETE /api/resume/items/{id} (문항 삭제)
- **변경** Settings extra="ignore" (.env 추가 필드 허용)
